...
from loris.lane_detection.advanced import  search_around_poly,find_lane_pixels, search_around_poly, non_zero, poly_fit, apply_poly
...
def gen_curvature_calculator(fit):
    ...
    def curvature(y_eval):
        ...
        curvature_radius = pow(1 + pow((2*fit[0]*y_eval + fit[1]), 2), 3/2) / (2*np.abs(fit[0]))
...
class Line():
    ...
    def __blend_fit(self, fit, max_y):
        ...
        self.bestx =  np.mean(np.array(self.recent_xfitted), axis=0) # update best x with the initialization
    ...
    def update_detection(self, nonzerox, nonzeroy, max_y, margin=100):
        ...
        y_range = np.arange(max_y)
        xs, ys, _ = search_around_poly(nonzerox, nonzeroy, self.current_fit, margin=margin)
        try:    
            self.__blend_fit(poly_fit(np.concatenate((ys, y_range)), np.concatenate((xs, apply_poly(self.current_fit, y_range) ))), max_y)
            self.pixels_x = xs
            self.pixels_y = ys  
        except Exception as exception:
...
class LaneDetector:
    ...
    def label_detection(self, warped_binary, margin_pixels=5):
        ...
        out_img[self.left.pixels_y, self.left.pixels_x] = [255, 0, 0] # left lane side in red
        out_img[self.right.pixels_y, self.right.pixels_x] = [0, 0, 255] # right lane side in blue
        ...
        out_img = warp(cv2.addWeighted(out_img, .8, green_lane, .2, 0, dtype=cv2.CV_32F).astype(np.uint8), 
                        rect_dict2nparray(dest), rect_dict2nparray(src))
    ...
    def label(self, orig, warped_binary, margin_pixels=5):
        ...
        out_img = cv2.addWeighted(orig, 0.7, self.label_detection(warped_binary, margin_pixels=margin_pixels), 0.5, 0, dtype=cv2.CV_32F).astype(np.uint8)
        ...
        cv2.putText(out_img, f"avg curv-rad at img-bottom({max_y}) {avg_curv_radius:.2f}", (20, 40), cv2.FONT_HERSHEY_SIMPLEX, 1, text_color, text_thickness)
    ...
    def process_image(self, image):
        ...
        if status:
            ...
            warped_binary = warp(threshold_pipeline(output), rect_dict2nparray(src), rect_dict2nparray(dest))

            if not self.left.detected or  not self.right.detected: # initialize lines
                leftx, lefty, rightx, righty = find_lane_pixels(255*warped_binary) # initialize the lane sides
                ...
                self.left.update_detection(nonzerox, nonzeroy, warped_binary.shape[0], margin=self.line_search_margin)
                self.right.update_detection(nonzerox, nonzeroy, warped_binary.shape[0], margin=self.line_search_margin)
            ...
            return self.label(output, warped_binary) # self.__label_detection(255*warped_binary)#
