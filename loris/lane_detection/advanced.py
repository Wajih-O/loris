...
def apply_poly(poly, y):
    ...
    return np.sum(
        np.array(
            [
                coef * (np.power(y, exp_))
                for exp_, coef in zip(reversed(range(len(poly))), poly)
            ]
        ),
        axis=0,
    ).astype(int)
...
def search_around_poly(nonzerox, nonzeroy, poly2, margin=100):
    ...
    inds = (
        nonzerox
        > (poly2[0] * (nonzeroy ** 2) + poly2[1] * nonzeroy + poly2[2] - margin)
    ) & (
        nonzerox
        < (poly2[0] * (nonzeroy ** 2) + poly2[1] * nonzeroy + poly2[2] + margin)
    )
...
def measure_curvature_pixels(left_fit, right_fit):
    ...
    def right_left_curvature(y_eval):
        ...
        left_curverad = pow(
            1 + pow((2 * left_fit[0] * y_eval + left_fit[1]), 2), 3 / 2
        ) / (2 * np.abs(left_fit[0]))
        right_curverad = pow(
            1 + pow((2 * right_fit[0] * y_eval + right_fit[1]), 2), 3 / 2
        ) / (2 * np.abs(right_fit[0]))
